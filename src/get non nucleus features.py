...
nonucleus_region = Image.open("op images/nonucleus_region.png")
#Load image in OpenCV (for LAB features)
nonucleus_regions = cv2.imread("op images/nonucleus_region.png")
...
df_RGB_nonu.to_csv('csv files/RGB_nonu.csv', index=False)
...
df_HSV_nonu.to_csv('csv files/HSV_nonu.csv', index=False)
...
cv2.imwrite('op images/imglab_nonu.jpg',imglab_nonu)#Save the obtained image
imglab_final_nonu = Image.open("op images/imglab_nonu.jpg") #Open the image (saving and reopening an image helps convert LAB values into a format that can be used up by the .getdata() function)
...
df_LAB_nonu.to_csv('csv files/LAB_nonu.csv', index=False)
...
np.save('npy files/nonucleus_features.npy', nonucleus) #save the obtained array as an .npy file for future use
