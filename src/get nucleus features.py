...
nucleus_region = Image.open("op images/nucleus_region.png")
#Load image in OpenCV (for LAB features)
nucleus_regions = cv2.imread("op images/nucleus_region.png")
...
df_RGB_nu.to_csv('csv files/RGB_nu.csv', index=False)
...
df_HSV_nu.to_csv('csv files/HSV_nu.csv', index=False)
...
cv2.imwrite('op images/imglab_nu.jpg',imglab_nu) #Save the obtained image
imglab_final_nu = Image.open("op images/imglab_nu.jpg") #Open the image (saving and reopening an image helps convert LAB values into a format that can be used up by the .getdata() function)
...
df_LAB_nu.to_csv('csv files/LAB_nu.csv', index=False)
...
np.save('npy files/nucleus_features.npy', nucleus) #save the obtained array as an .npy file for future use
